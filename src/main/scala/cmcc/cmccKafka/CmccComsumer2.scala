...
import cmcc.utils.{CaculateTools, ConfUtil}
 ...
 //   sparkConf.set("spark.streaming.kafka.maxRatePerPartition", "100")
  ...
  //  sparkConf.set("spark.streaming.stopGracefullyOnShutdown", "true")
    ...
    stream.foreachRDD(rdd => {
      ...
      Cmcckpi.Kpi_jichu(baseData)
      Cmcckpi.Kpi_shishi(baseData)

      Cmcckpi.Kpi_faliure(baseData, pcode2pname: Broadcast[Map[String, AnyRef]])

      //每分钟实时充值情况分布
      // (日期, 小时,分钟， Kpi(订单，成功订单，订单金额，订单时长),省份)
      Cmcckpi.kpi_quality(baseData)
    })
